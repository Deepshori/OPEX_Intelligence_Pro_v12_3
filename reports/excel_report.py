...
def add_dashboard(writer, workbook, data, title_fmt, header_fmt, money_fmt):
    ...
    ws.write("A1", "OPEX Intelligence Pro v12.3 Dashboard", title_fmt)
    ...
    if monthly is not None and not monthly.empty:
        ...
        if not overall.empty:
            start_row = 35
            start_col = 3
            cols = ["Month", "Projected YTD Budget", "Projected Actual Expense", "Projected YTD Variance"]
            ...
            fchart.add_series({"name": "Projected Budget", "categories": ["Dashboard", start_row + 1, start_col, start_row + len(overall), start_col], "values": ["Dashboard", start_row + 1, start_col + 1, start_row + len(overall), start_col + 1]})
            fchart.add_series({"name": "Projected Actual", "categories": ["Dashboard", start_row + 1, start_col, start_row + len(overall), start_col], "values": ["Dashboard", start_row + 1, start_col + 2, start_row + len(overall), start_col + 2]})
...
def add_charts(writer, workbook, data):
    ...
    if monthly is not None and not monthly.empty:
        ...
        if not overall.empty:
            ...
            vchart.add_series({"name": ["PPT Chart Data", 0, base_col + 3], "categories": ["PPT Chart Data", 1, base_col, rows_m, base_col], "values": ["PPT Chart Data", 1, base_col + 3, rows_m, base_col + 3]})
            vchart.set_title({"name": "Projected Monthwise Variance"})
